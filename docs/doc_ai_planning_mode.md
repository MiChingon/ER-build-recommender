...
This document defines how the assistant must operate when invoked in planning mode for this repository.
...
5. Always read [`docs/doc_architecture.md`](./doc_architecture.md) before proposing any detailed plan.
...
13. The detailed task brief may refine execution steps, but must not introduce new behavior-affecting decisions (data shape, file placement, error handling, observability) that were not approved in the High-Level Technical Contract.

## Standard Workflow

1. Restate objective in one short paragraph.
2. Read [`docs/doc_architecture.md`](./doc_architecture.md) and gather context (existing files, architecture, constraints).
...
- Public contract impact (exported types, shape of `Recommendation` / `Weapon` / etc.)
- Exact input/output shape
- Backward compatibility statement
- Architectural placement and ownership (which layer: `data/`, `lib/`, `pages/components/`, `common/components/`)
- Architectural Delta
- Artifact inventory
- Source of truth
- Mapping ownership
...
5. Naming must be consistent across the contract:
   - The same component, function, or type must have a single name.
...
Failure: if two engineers could implement it differently, the contract is invalid.

## Data Contract Closure Rule (Mandatory)

All externally visible data shapes (exported types, return values of `recommend(...)`, `Weapon.scalingTable`, etc.) defined in the High-Level Technical Contract must be fully deterministic and unambiguous.

For every field in any output object, the contract must explicitly define:

1. Presence: required vs nullable.
2. Source of truth: exact origin (data file, computed value, derived field).
3. Missing data behavior: must return `null`, empty string, empty array, or sentinel.
4. Transformation rules: pass-through vs transformed.
5. Prohibition of synthesis: no field may be inferred unless explicitly approved.
...
- "optional depending on data file"

If a field is listed in the contract, its behavior must be fully defined even when data is missing.

If an example value is shown in the High-Level Technical Contract, it must be fully aligned with the approved field list, nullability rules, and source-of-truth rules. No extra example-only fields are allowed.

Failure condition: if two valid implementations could produce different values for the same field given the same input, the contract is not closed.
...
6. `Validation strategy` in the High-Level Technical Contract must stay scenario-based; exact commands and the manual-browser checklist belong in the detailed task brief.
7. Include exact function or hook names only when they are required for architectural approval or to prevent implementation drift.
...
- Page-level changes: exact components in `src/pages/` to create, modify, or reuse.
- Recommender changes: exact functions in `src/lib/recommender.ts` to create, modify, reuse, or leave unchanged.
- Data changes: exact files in `src/data/` to extend (hand-curated) or regenerate (machine-generated).
- Shared-primitive changes: exact components in `src/common/components/` affected, or an explicit statement that none are changed.
- State-shape changes: exact fields added to `BuildPickerProps` / state container, or an explicit statement that none are.
- Verification impact: which manual-browser flows and which `tsc -b` / `npm run doctor` outcomes are expected.
- Ownership boundaries: where each new responsibility lives; where it must not live.
- Reuse statement: which existing path remains the source of truth; which new component only wraps or composes that path.
...
The contract must also include an explicit approval boundary: after approval, the detailed task brief may expand implementation and verification steps, but may not introduce new behavior-affecting decisions.
...
1. Identify impacted layer(s) and keep responsibilities clean (data vs lib vs pages vs common).
2. Apply minimal changes in the correct module.
3. Update imports/callers in the same task.
4. Run `npm run build` (which chains `tsc -b && vite build`) to catch type errors.
5. Run `npm run doctor` to catch React-pattern, performance, and architecture regressions.
6. Start the dev server (`npm run dev`, port 5179) and manually exercise the affected flow.
7. Report:
   - files changed
   - contract impact
...
4. Define the exact source of truth for runtime behavior (one entry point, one state shape, one set of constants).
...
5. Observability: the plan states what to surface in the rationale array / analytics event when applicable.
...
11. Reviewer independence: a senior engineer can identify the planned components, recommender helpers, data changes, state shape changes, and manual verification impact without reading the detailed task brief.
...
2. Prefer positive specification ("the recommender returns X") over historical phrasing ("do not return Y").
...
When a plan depends on third-party libraries (jsPDF, MUI APIs, etc.):

1. Verify expected input/output formats against official docs before locking plan details.
2. Do not assume file or component shapes are supported unless documented.
3. If a custom wrapper/adaptation is needed, state it explicitly in the plan.
...
1. Every planning workflow must include at least one concrete verification path. For this codebase, the canonical paths are:
   - `npm run build` (tsc + vite production build)
   - `npm run doctor` (`npx react-doctor@latest`)
   - Manual smoke test via `npm run dev` and a browser checklist (see [`docs/doc_verification_guide.md`](./doc_verification_guide.md)).
...
4. In the detailed task brief, validation must specify:
   ...
   - validation type (`required` vs `optional`) and prerequisites
...
2. `Architectural Delta` is the canonical location for ownership, reuse, no-change statements, and verification impact.
...
- [ ] New code is in the correct layer (`data/`, `lib/`, `pages/components/`, `common/components/`).
- [ ] Naming follows the conventions in [`docs/doc_architecture.md`](./doc_architecture.md).
- [ ] Existing exported shapes (`Recommendation`, `Weapon`, etc.) are preserved unless the contract approved a change.
- [ ] No React imports introduced into `src/lib/`.
- [ ] No hand-edits to machine-generated data files.
- [ ] Imports and callers updated.
- [ ] `npm run build` clean.
- [ ] `npm run doctor` clean (or remaining warnings are pre-approved FPs).
- [ ] Relevant manual-browser smoke check executed.
...
- [ ] T1.1 Inspect `Recommendation` shape
- [ ] T3.2 Add `equipLoadProfile` field
- [ ] T6.4 Reuse `GearTile` for the new slot
